...
int main(){
  ...
  printf("TEST 6 RET = %d\n", insert_filled_rows(ledger, entries, -5,3));
  print_ledger_to_stream(ledger, stdout);  
  print_summary_to_stream(ledger, stdout, 1);

  printf("TEST 7 RET = %d\n", insert_filled_rows(ledger, entries, 5000,3));
  print_ledger_to_stream(ledger, stdout);  
  print_summary_to_stream(ledger, stdout, 1);
  free_ledger(&ledger);

  get_ledger(&ledger, "../data/ledgers/empty.txt", NULL, NULL);
  printf("TEST 10 RET = %d\n", insert_filled_rows(ledger, entries, 0,3));
  print_ledger_to_stream(ledger, stdout);  
  print_summary_to_stream(ledger, stdout, 1);
  free_ledger(&ledger);
