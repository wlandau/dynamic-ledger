 ...
 *          of a Ledger object. Specifically, 
 *          ledger->entries[coords[i][0]][coords[i][1]] 
 *          is replaced with "entry", for i from 0 to length(coords) - 1.
 *          Same as map_by_col, but different data storage format. Set "append"
 *          to 0 to overwrite each entry, 1 to append to the head of each entry,
 *          and 2 to append to the tail of each entry. 
 */

err_t map_by_row(Ledger *ledger, char *entry, int *coords, int howmany, int append){
  int i;
  for(i = 0; i < howmany; ++i)
