...
/**
 * @details Map a single character string to a rectangular region of the spreadsheet
 *          of a Ledger object. Specifically, ledger->entries[fields[i]][rows[j]] 
 *          is replaced with "entry", for i from 0 to length(fields) - 1, 
 *          j from 0 to length(rows) - 1. Set "append" to 0 to overwrite
...
err_t map_to_grid(Ledger *ledger, char *entry, int *rows, int nrows,
  ...
  if(entry == NULL){
    for(i = 0; i < nfields; ++i)
      for(j = 0; j < nrows; ++j){
        if(fields[i] < 0 || fields[i] >= NFIELDS ||
           rows[j] < 0 || rows[j] >= ledger->nrows)
          continue;
        if(!append)
          strcpy(ledger->entries[fields[i]][rows[j]], NIL);
      }
